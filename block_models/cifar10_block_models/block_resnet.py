...
def block_conv3x3(in_planes, out_planes, block_size=0, stride=1, groups=1, dilation=1):
    """3x3 block convolution with padding"""
    return BlockConv2d(in_planes, out_planes, kernel_size=3, block_size=block_size, stride=stride,
                     padding=dilation, groups=groups, bias=USE_BIAS, dilation=dilation)


def block_conv1x1(in_planes, out_planes, block_size=0, stride=1):
    """1x1 block convolution"""
    return BlockConv2d(in_planes, out_planes, kernel_size=1, block_size=block_size, stride=stride, bias=USE_BIAS)
...
class BlockBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, block_size=0, stride=1, downsample=None, groups=1,
        ...
        #if stride == 1:
        self.conv1 = block_conv3x3(inplanes, planes, block_size, stride)
       # else:
        #    self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = block_conv3x3(planes, planes, block_size)
        self.bn2 = norm_layer(planes)
        self.downsample = downsample
        self.stride = stride

    def forward(self, x, use_block=True):
        identity = x

        out = self.conv1(x, use_block)
        out = self.bn1(out)
        out = self.relu(out)
        ...
        if self.downsample is not None:
            x = self.downsample[0](x, use_block)
...
class BlockBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, block_size=0, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = block_conv1x1(inplanes, width, block_size)
        self.bn1 = norm_layer(width)
        self.conv2 = block_conv3x3(width, width, block_size, stride, groups, dilation)
        self.bn2 = norm_layer(width)
        self.conv3 = block_conv1x1(width, planes * self.expansion, block_size)
...
class BlockResNet(nn.Module):
    ...
    def __init__(self, block, layers, block_size=0, num_classes=10, zero_init_residual=False,
        ...
        self.conv1 = BlockConv2d(3, self.inplanes, kernel_size=3, block_size=0, stride=1, padding=1, bias=USE_BIAS)
        ## END
    ...
    def _make_layer(self, block, planes, blocks, block_size=0, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                block_conv1x1(self.inplanes, planes * block.expansion, block_size, stride),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, block_size, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, block_size, groups=self.groups,
...
def _block_resnet(arch, block, layers, pretrained, progress, device, block_size, **kwargs):
    model = BlockResNet(block, layers, block_size, **kwargs)
    if pretrained:
        arch = arch.replace('block_','')
        state_dict = torch.load('block_models/cifar10_block_models/'+arch+'.pt', map_location=device)
        model.load_state_dict(state_dict)
    return model


def block_resnet18(pretrained=False, progress=True, device='cpu', block_size=0, **kwargs):
